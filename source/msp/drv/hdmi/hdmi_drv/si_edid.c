...
extern GPIO_EXT_FUNC_S *gpio_func_ops;
...
static HI_U32 SI_DDC_Adjust(void)
{
    ...
    //when DDC_DELAY_CNT = 0x64  oscclk = 50Mhz  dscl = 50Mhz / (DDC_DELAY_CNT * 8) = 62500 hz
    // dscl should in range 48Khz ~ 72Khz
    //DRV_HDMI_WriteRegister(0xf8ce03d8, 0x64); //DDC_DELAY_CNT
    //第一版本的手册上建议值
    //50khz
    //DRV_HDMI_WriteRegister(0xf8ce03d8, 0x28); //DDC_DELAY_CNT
    // 硬件说高速I2C范围在 100Khz ~ 400Khz 
    //100khz
    //DRV_HDMI_WriteRegister(0xf8ce03d8, 0x14); //DDC_DELAY_CNT

    // 75Khz
    DRV_HDMI_WriteRegister(0xf8ce03d8, 0x1a); //DDC_DELAY_CNT
    // 63Khz
    ...
    //bit 6  bus_low
    DRV_HDMI_ReadRegister(0xf8ce03c8, &value);
    HI_INFO_HDMI("DDC Status(0x72:0xf2):0x%x\n", value);

    DRV_HDMI_ReadRegister(u32BaseAddr + 0x04, &value);
    HI_INFO_HDMI("HDMI PINSHARE Tx_SDA:0x%x is:0x%x\n", u32BaseAddr + 0x04, value);
    
    DRV_HDMI_ReadRegister(u32BaseAddr + 0x00, &value);
    HI_INFO_HDMI("HDMI PINSHARE Tx_SCL:0x%x is:0x%x\n", u32BaseAddr + 0x00, value);
    ...
    if(value == 0x01)
    {
        ...
        DRV_HDMI_WriteRegister(u32BaseAddr + 0x00,(HI_U32)0x00);
        DRV_HDMI_WriteRegister(u32BaseAddr + 0x04,(HI_U32)0x00);
        ...
        Ret = DRV_HDMI_WriteRegister(u32BaseAddr + 0x00,(HI_U32)0x01);
        DRV_HDMI_WriteRegister(u32BaseAddr + 0x04,(HI_U32)0x01);
    }
#endif /*--NO MODIFY : COMMENT BY CODINGPARTNER--*/
...
#if 0 /*--NO MODIFY : COMMENT BY CODINGPARTNER--*/
    //DSCA DSCL 管脚复用 和 GPIO模拟信号拉高
    HI_U32 u32BaseAddr = 0x10203020;
    //HI_CHIP_TYPE_E enChip;
    //HI_CHIP_VERSION_E enChipVersion;
    //HI_DRV_SYS_GetChipVersion(&enChip, &enChipVersion); 

    DRV_HDMI_ReadRegister(0x101703c8, &value);
    HI_INFO_HDMI("DDC Status(0x7a:0xf2):0x%x\n", value);
    
    DRV_HDMI_ReadRegister(u32BaseAddr, &value);
    HI_INFO_HDMI("u32BaseAddr:0x%x is:0x%x\n", u32BaseAddr, value);
    ...
    {
        //PIO Mode
        Ret = DRV_HDMI_WriteRegister(u32BaseAddr, (HI_U32)0x00);
        if((enChipVersion == HI_CHIP_VERSION_V300)||(HI_CHIP_TYPE_HI3712 == enChip))
        {
        	SI_GPIO_SetDirBit(0, 3, HI_TRUE);
	        SI_GPIO_SetDirBit(0, 4, HI_FALSE); //GPIO0_4:HDMI_SCL
	        SI_GPIO_ReadBit(0, 3, &value);//GPIO0_3:HDMI_SDA
	        ...
	        {
	            HI_INFO_HDMI("***Pull up SCL Voltage for DDC, count:%d\n", count);
	            SI_GPIO_WriteBit(0, 4, HI_TRUE);//GPIO0_4:HDMI_SCL
	            mdelay(1);
	            SI_GPIO_WriteBit(0, 4, HI_FALSE);//GPIO0_4:HDMI_SCL
	            mdelay(1);
	            SI_GPIO_ReadBit(0, 3, &value);//GPIO0_3:HDMI_SDA
	        }
        }
        else
        {
	        ...
	        {
	            HI_INFO_HDMI("***Pull up SCL Voltage for DDC, count:%d\n", count);
	            //Ret = HI_UNF_GPIO_WriteBit(25, HI_TRUE);//GPIO3_1:HDMI_SCL
	            SI_GPIO_WriteBit(3, 1, HI_TRUE);
	            mdelay(1);//msleep(1);
	            //Ret = HI_UNF_GPIO_WriteBit(25, HI_FALSE);//GPIO3_1:HDMI_SCL
	            SI_GPIO_WriteBit(3, 1, HI_FALSE);
	            mdelay(1);//msleep(1);
        ...
        if (value == 0)
        {
            HI_WARN_HDMI("Can not pull up SCL Voltage\n");
        }
        
        //SDA/SCL Mode
        Ret = DRV_HDMI_WriteRegister(u32BaseAddr, (HI_U32)0x01);
    }
#endif /*--NO MODIFY : COMMENT BY CODINGPARTNER--*/
    return HI_SUCCESS;
}
...
static HI_U8 ParsingVideoDATABlock(HI_U16 * pCRC16, HI_U8 BaseAddr)
{
    ...
    if(!Error)
    {
        ...
        for(i = 0 ; i < 4; i++)
        {
            if(NBytes <= 8)
            {
                Error = SI_BlockReadEDID(NBytes, BaseAddr + i*8, Data);
                if(!Error)
                {
                    SaveEdidToEeprom(BaseAddr  + i*8, NBytes, Data);
...
{    
    ...
    SI_BlockReadEDID(128, 0, EDID_Basic_Block);
...
HI_U8 SI_EDID_Force_Setting(void)
{
   ...
   // memset(&g_stSinkCap, 0, sizeof(HI_UNF_HDMI_SINK_CAPABILITY_S));
    ...
    {
        HI_INFO_HDMI("Force to HDMI Mode\n");
