...
static const HI_CHAR driver_name [] = DRIVER_NAME;
static struct class *hivdec_class;
static dev_t hivdec_devno;
struct vdec_entry* the_vdec = NULL; 
...
{
	return HI_DRV_VPSS_GlobalInit();
}

static inline HI_S32 vdec_exit_with_vpss(HI_VOID)
{
	return HI_DRV_VPSS_GlobalDeInit();
}
...
HI_U32 get_channel_num(struct vdec_entry *vdec)
{
	...
	if (!vdec || (vdec->total_chan_num >= MAX_CHAN_NUM)) 
       {
              OmxPrint(OMX_FATAL, "%s: vdec = NULL / total_chan_num(%d) invalid\n", __func__, vdec->total_chan_num);
		return -EINVAL;
	}
...
static HI_S32 hivdec_setup_cdev(struct vdec_entry *vdec, const struct file_operations *fops)
{
	...
	dev = device_create(hivdec_class, NULL, hivdec_devno, NULL, DRIVER_NAME);
	if (IS_ERR(dev)) 
       {
		rc = PTR_ERR(dev);
...
static long hivdec_ioctl(struct file *fd, unsigned int code, unsigned long arg)
{
	...
	case VDEC_IOCTL_CHAN_GET_MSG:
	{
		...
		if (!pchan->ops || !pchan->ops->get_msg)
              {      
                     OmxPrint(OMX_FATAL, "%s(), l%d: case condition invalid!\n", __func__, __LINE__);
			return -EFAULT;
              }
		...
		if (status == -ESHUTDOWN) 
              {
                     OmxPrint(OMX_WARN, "%s(), L%d: status = -ESHUTDOWN\n", __func__, __LINE__);
			return status;
		}
		else if (status < 0) 
        {
            OmxPrint(OMX_INFO, "%s(), l%d: no msg found\n", __func__, __LINE__);
			return -EFAULT;
		}
...
static HI_S32 __devinit hivdec_probe(struct platform_device * pltdev)
{
	...
	spin_lock_init(&vdec->lock); // yyc test
...
HI_S32 __init hivdec_init(HI_VOID)
{
       HI_S32 ret;
	...
	if(ret < 0)
       {
	       OmxPrint(OMX_FATAL, "%s call omx_vdec_proc_init failed!\n", __func__);    
		goto exit1;
       }
    
#ifndef CONFIG_SUPPORT_CA_RELEASE
    printk("Load hi_omxvdec.ko success.\t(%s)\n", VERSION_STRING);
#endif
...
HI_VOID __exit hivdec_exit(HI_VOID)
{
	platform_driver_unregister(&hivdec_driver);
	platform_device_unregister(&hivdec_device);

       channel_proc_exit();  //yyc test

#ifndef CONFIG_SUPPORT_CA_RELEASE
    printk("Unload hi_omxvdec.ko success.\t(%s)\n", VERSION_STRING);
#endif

}


module_init(hivdec_init);
module_exit(hivdec_exit);
