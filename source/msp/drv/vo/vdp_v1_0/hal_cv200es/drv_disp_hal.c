...
#define DISP_VTTHD_DISP0_TO_DISP1 24
...
HI_S32 PF_SetWbcIORect(DISP_WBC_E eWbc, HI_RECT_S *in, HI_RECT_S *out)
{
    ...
    /*in cv200es, 4 tap selected in wbc zme and 6 taps in v0, 8 tap in vpss,
      in cv200es, fixed 4 tap in both wbc zme and v0, vpss remain the same witch cv200es.*/
    ...
    if (stZmeO.u32ZmeRatioHL) {
        VDP_WBC_SetZmeCoefAddr(VDP_LAYER_WBC_HD0, VDP_WBC_PARA_ZME_HOR, stZmeO.u32ZmeCoefAddrHL);
    ...
    if (stZmeO.u32ZmeRatioVL) {
        VDP_WBC_SetZmeCoefAddr(VDP_LAYER_WBC_HD0, VDP_WBC_PARA_ZME_VER, stZmeO.u32ZmeCoefAddrVL);
