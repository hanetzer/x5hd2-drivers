...
volatile HI_U32 *pu32VdacReg = HI_NULL;
volatile HI_U32 *pu32VdpSysCtrlReg = HI_NULL;
#if 0
...
{
    pu32VdacReg = (HI_U32 *)virAddr;
}


HI_VOID  VDPSYSCTRL_DRIVER_SetVirtualAddr(HI_U32 virAddr)
{
    pu32VdpSysCtrlReg = (HI_U32 *)virAddr;
}
...
#define VDAC_SYSCTRL_RESET_VALUE 0xF3ul
HI_VOID  VDAC_DRIVER_Initial()
{
    // set vdac reset value
    pu32VdacReg[0] = VDAC_SYSCTRL_RESET_VALUE;
...
HI_VOID VDP_DHD_DEFAULT( )
{
   ...
   volatile U_DHD0_SYNC_INV DHD0_SYNC_INV;
    ...
    DHD0_CTRL.u32 = 0x80008011;//0x8d0f8000;//0x8ad20011;
...
HI_VOID VDP_DHD1_DEFAULT()
{
   ...
   volatile U_DHD0_SYNC_INV DHD1_SYNC_INV;
    ...
    DHD1_CTRL.u32 = 0x80000001;//0x8ad20011;
...
{
    //基地址:0xF8A2_2000
    //0xF8A2211C  0x000000f3 //(7\6\5\4\1\0)
    pu32VdacReg[0] = VDAC_SYSCTRL_RESET_VALUE;
    //printk("xxxxxxxxxxxxxxx pu32VdacReg = 0x%x\n", (HI_U32)pu32VdacReg);
    ...
    */
    VDP_RegWrite((HI_U32)(&(pVdpReg->VO_DAC_CTRL.u32)), 0x00004000); 
#if 0
    VDP_RegWrite((HI_U32)(&(pVdpReg->VO_DAC_C_CTRL.u32)), 0x10000000); 
    VDP_RegWrite((HI_U32)(&(pVdpReg->VO_DAC_R_CTRL.u32)), 0x10000000); 
    VDP_RegWrite((HI_U32)(&(pVdpReg->VO_DAC_G_CTRL.u32)), 0x10000000); 
    VDP_RegWrite((HI_U32)(&(pVdpReg->VO_DAC_B_CTRL.u32)), 0x10000000); 
#endif
    ...
    */
    ...
    //VDP_RegWrite((HI_U32)(&(pVdpReg->VO_DAC_CTRL.u32)), 0x80004000); 
    DISP_MSLEEP(10);
    VDP_RegWrite((HI_U32)(&(pVdpReg->VO_DAC_CTRL.u32)), 0x8000c000); 
    VDP_RegWrite((HI_U32)(&(pVdpReg->VO_DAC_CTRL.u32)), 0x80008000); 
    DISP_MSLEEP(1);
...
#define X5HD2_VDAC0_MASK_RESET_OVER   0xE3ul
#define X5HD2_VDAC0_MASK_RESET        0x10ul

#define X5HD2_VDAC1_MASK_RESET_OVER   0xD3ul
#define X5HD2_VDAC1_MASK_RESET        0x20ul

#define X5HD2_VDAC2_MASK_RESET_OVER   0xB3ul
#define X5HD2_VDAC2_MASK_RESET        0x40ul

#define X5HD2_VDAC3_MASK_RESET_OVER   0x73ul
#define X5HD2_VDAC3_MASK_RESET        0x80ul

HI_VOID VDP_VDAC_ResetFmt(HI_U32 uVdac, HI_DRV_DISP_FMT_E enFmt)
{
    HI_U32 vdacvalue = 0x02000000ul;
    
    /* 720P 1080I, 0xc2000000ul;
       1080p50,    0xc0000000ul 
       PAL,        0xc6000000ul */
    
    if (enFmt <= HI_DRV_DISP_FMT_1080P_50)
    {
        vdacvalue = 0x00000000ul;
    }

    if (enFmt >= HI_DRV_DISP_FMT_PAL)
    {
        vdacvalue = 0x06000000ul;
    }

    //printk("VDP_VDAC_ResetFmt  vdac=%d, fmt=%d\n", uVdac, enFmt);
        
    switch(uVdac)
    {
        case 0:
        /*
        基地址：0xF8CC_0000  //channel C
        0xF8CC0130   0x80000000 // enable 11V (31)
        0xF8CC0130   0xc2000000 // tv format select 720p (31\30\25)
        
        基地址：0xF8A2_2000
        0xF8A2211C   0x000000e3  //release rstn_c (7\6\5\1\0)
        */
            pu32VdacReg[0] = pu32VdacReg[0] | X5HD2_VDAC0_MASK_RESET;
            //VDP_RegWrite((HI_U32)(&(pVdpReg->VO_DAC_C_CTRL.u32)), 0x80000000); 
            VDP_RegWrite((HI_U32)(&(pVdpReg->VO_DAC_C_CTRL.u32)), vdacvalue); 

            //pu32VdacReg[0] = pu32VdacReg[0] & X5HD2_VDAC0_MASK_RESET_OVER;
            break;

        case 1:
            pu32VdacReg[0] = pu32VdacReg[0] | X5HD2_VDAC1_MASK_RESET;

            //VDP_RegWrite((HI_U32)(&(pVdpReg->VO_DAC_R_CTRL.u32)), 0x80000000); 
            VDP_RegWrite((HI_U32)(&(pVdpReg->VO_DAC_R_CTRL.u32)), vdacvalue); 
        /*
        基地址：0xF8A2_2000
        0xF8A2211C   0x000000c3 //release rstn_r (7\6\1\0)
        */
            //pu32VdacReg[0] = pu32VdacReg[0] & X5HD2_VDAC1_MASK_RESET_OVER;
            break;
        case 2:
            pu32VdacReg[0] = pu32VdacReg[0] | X5HD2_VDAC2_MASK_RESET;

            //VDP_RegWrite((HI_U32)(&(pVdpReg->VO_DAC_G_CTRL.u32)), 0x80000000); 
            VDP_RegWrite((HI_U32)(&(pVdpReg->VO_DAC_G_CTRL.u32)), vdacvalue); 

        /*
        基地址：0xF8A2_2000
        0xF8A2211C   0x00000083 //release rstn_r (7\1\0)
        */
            //pu32VdacReg[0] = pu32VdacReg[0] & X5HD2_VDAC2_MASK_RESET_OVER;
            break;
        case 3:
            pu32VdacReg[0] = pu32VdacReg[0] | X5HD2_VDAC3_MASK_RESET;

            //VDP_RegWrite((HI_U32)(&(pVdpReg->VO_DAC_B_CTRL.u32)), 0x80000000); 
            VDP_RegWrite((HI_U32)(&(pVdpReg->VO_DAC_B_CTRL.u32)), vdacvalue); 

        /*
        基地址：0xF8A2_2000
        0xF8A2211C   0x00000003 //release rstn_r (\1\0)
        */
            //pu32VdacReg[0] = pu32VdacReg[0] & X5HD2_VDAC3_MASK_RESET_OVER;
            break;
...
static HI_U32 s_VDACLink[2][HI_DRV_DISP_VDAC_SIGNAL_BUTT] = 
{
{4, 4, 4, 5, 6, 4, 4, 7, 8, 9, 8},
...
//-------------------------------------------------------------------
// VIDEO LAYER CONFIG
HI_VOID  VDP_VID_SetInReso2(HI_U32 u32Data, HI_RECT_S *pstRect)
{
    U_V0_IRESO V0_IRESO;
    U_V0_CPOS  V0_CPOS;
    ...
    /*FIXME: pay attention please, here crop is a tempary modify, we should find a final location.*/
    V0_CPOS.u32 = VDP_RegRead((HI_U32)(&(pVdpReg->V0_CPOS.u32) + u32Data * VID_OFFSET));
    V0_CPOS.bits.src_xfpos = 0;
    V0_CPOS.bits.src_xlpos = V0_IRESO.bits.iw;
    VDP_RegWrite((HI_U32)(&(pVdpReg->V0_CPOS.u32) + u32Data * VID_OFFSET), V0_CPOS.u32);     
...
HI_VOID  VDP_VID_SetZmeFirEnable2(HI_U32 u32Data, HI_U32 u32bEnable)
{
    U_V0_HSP V0_HSP;
    U_V0_VSP V0_VSP;
