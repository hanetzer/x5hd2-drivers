...
{
    printk("\nusage as following: \n");
    printk("    cat /proc/graphics/higo              display all proc information \n");
    printk("    echo proc on > /proc/graphics/higo   enable higo proc. default is disable!\n");
    printk("    echo proc off > /proc/graphics/higo  disable the proc information! \n");
    printk("    echo time on > /proc/graphics/higo   enable higo print time. default is disable!\n");
    printk("    echo time off > /proc/graphics/higo  disable print time! \n");
}
...
HI_S32 Log_Ioctl(struct seq_file *p, HI_U32 cmd, HI_U32 arg)
{
...
HI_S32 HIGO_Log_Init(HI_VOID)
{
    HI_S32 s32Ret = HI_FAILURE;
    DRV_PROC_EX_S  proc;
    s_u32LogMemPhyAddr = hifb_buf_allocmem("higo_log", HIGO_LOG_MEM_SIZE);
    ...
    proc.fnRead   = Log_Show;
    proc.fnWrite  = Log_Write;
    proc.fnIoctl  = Log_Ioctl;
        
    HI_DRV_PROC_AddModuleEx("higo", &proc, NULL);
    return HI_SUCCESS;
