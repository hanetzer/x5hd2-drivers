 ...
 **/
METHODDEF(void)
ycc_bgra_8888_convert (j_decompress_ptr cinfo,
         JSAMPIMAGE input_buf, JDIMENSION input_row,
         JSAMPARRAY output_buf, int num_rows)
{
 ...
 **/
METHODDEF(void)
rgb_bgra_8888_convert (j_decompress_ptr cinfo,
         JSAMPIMAGE input_buf, JDIMENSION input_row,
         JSAMPARRAY output_buf, int num_rows)
{
	          ...
	          JSAMPARRAY output_buf, int num_rows)
{
		  ...
		  /*** 设置内存模式时，我做了一次拷贝，输出的是YUVYUVYUVYUV的***/
		  //my_cconvert_ptr cconvert = (my_cconvert_ptr) cinfo->cconvert;
		  register int y, cb, cr;
		  register JSAMPROW outptr;
		  register JSAMPROW inptr0, inptr1, inptr2;
 ...
 **/
METHODDEF(void)
gray_bgra_8888_convert (j_decompress_ptr cinfo,
          JSAMPIMAGE input_buf, JDIMENSION input_row,
          JSAMPARRAY output_buf, int num_rows)
{
...
METHODDEF(void)
gray_bgr_565_convert (j_decompress_ptr cinfo,
          JSAMPIMAGE input_buf, JDIMENSION input_row,
          JSAMPARRAY output_buf, int num_rows)
{
	  ...
	  switch (cinfo->out_color_space)
	  {
		  ...
		  case JCS_GRAYSCALE:
		   ...
		   case JCS_BGRA_8888:
			    ...
			    if (cinfo->jpeg_color_space == JCS_YCbCr) 
				{
			      cconvert->pub.color_convert = ycc_bgra_8888_convert;
			      build_ycc_rgb_table(cinfo);
				  
			    } else if (cinfo->jpeg_color_space == JCS_GRAYSCALE)
			    {
			      cconvert->pub.color_convert = gray_bgra_8888_convert;
			    }
				else if (cinfo->jpeg_color_space == JCS_RGB)
				{
			      cconvert->pub.color_convert = rgb_bgra_8888_convert;
			    } 
				else
				{
			       ERREXIT(cinfo, JERR_CONVERSION_NOTIMPL);
		   ...
		   case JCS_BGR_565:
			    ...
			    if (cinfo->dither_mode == JDITHER_NONE)
				{
				  ...
				  {
			        cconvert->pub.color_convert = gray_bgr_565_convert;
			      } 
				  else if (cinfo->jpeg_color_space == JCS_RGB) 
				  {
